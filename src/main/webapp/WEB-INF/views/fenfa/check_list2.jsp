...
<body>
									...
									<div class="row">
										<div class="col-sm-2 col-sm-offset-10"
											...
											<select id="basic" class="form-control selectpicker" onchange="getOrderPage(1)">
												...
												<option value="0">市场人员未审核</option>
												<option value="1" selected="selected">实施管理人员未审核</option>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">实施服务单审核</h4>
				</div>
	...
	<script type="text/javascript">
						...
						success : function(data) {
							...
							if (orders.length == 0) {
								...
								$.each(orders,function(index){
									...
									if(status=='实施管理人员未审核'){
										tbody = tbody + "<td><a href=\"#\" onclick=\"forceCheck('"+orders[index].id+"', true);\"><i class=\"glyphicon glyphicon-eye-open\"></i> 审核</a></td></tr>";
									}else{
							...
							if (pageNum > 1) {
								ul = ul
										+ "<li><a href=\"javascript:getOrderPage(1)\">首页</a></li>";
							}
							if (pageNum - 1 > 1) {
								ul = ul
										+ "<li><a href=\"javascript:getOrderPage("
										+ pageNum + "-1)\">上页</a></li>";
							}
							...
							if (pageNum < totalPageNum) {
								ul = ul
										+ "<li><a href=\"javascript:getOrderPage("
										+ totalPageNum + ")\">尾页</a></li>";
							}
							$("#pagingBar").html(ul);
							var pageNumLabel = "";
							if (orders.length > 0) {
								pageNumLabel = "<label>第" + pageNum + "页/共"
										+ totalPageNum + "页</label>";
		...
		function forceCheck(id) {
			$.get("${ctx}/main/findCompanyAndServicesByOrderId?orderId=" + id,
		...
		//         	$(".sidebar ul li").eq(6).addClass("active");
		//         	$(".sidebar ul li a").eq(6).css("background-color","#636A71");
		//         	$(".sidebar ul li a").eq(6).css("color","#FFF");
	</script>
	<script id="peopleTmpl" type="text/x-jsrender">
<form class="form-horizontal" role="form" id="myform1" action="${ctx}/main/checkOrder2">
<input type="hidden" name="id" value="{{:id}}"/>
<h5>&nbsp;&nbsp;&nbsp;&nbsp;产品名称:{{:order.appname}}<input type="hidden" name="status" value="2"/></h5>
      ...
      <div class="panel-body">
       <div class="row">
		...
		<div class="row">
			...
			<label class="col-sm-3 text-right">负责人电话</label>
      		<div class="col-sm-3">{{:company.headerPhone}}</div>
   		</div>
		<div class="row">
      		<label class="col-sm-2 text-right">联系电话</label>
      		<div class="col-sm-4">{{:company.phone}}</div>
			<label class="col-sm-3 text-right">邮箱</label>
      		<div class="col-sm-3">{{:company.headerEmail}}</div>
   		</div>
