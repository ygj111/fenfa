function workflow() {
	...
	var getFlowList = function(pageNo, param) {
		$.get("../admin/flow/list?pageNo=" + pageNo + "&displayName=" + param,
		...
		$.get("../admin/flow/orders/"+pageno+"/pagesize/"+pagesize, function(data, status){
			if("success" == status){
				var table = {title : "流程实例"};
					...
					sysmanager(table);
					$("#sys_toolbar").remove();
					fillfloworderlist(data, pageno);
			}
		});
	};
	var fillfloworderlist = function(data, pageno){
		...
		$(".toview-floworder").click(function(){
			...
			var order={"title":"查看流程实例","processId":row.process.id ,"orderId":row.id};
			html = $.templates.flowordertable.render(order);
			$("#main_table").hide();
